...
public final class Archive7zTest extends ArchiveContract<Archive7zKind>
{
  @Override @Nonnull Archive<Archive7zKind> getArchive(
      ...
      FilesystemError
  {
    ...
    final PathReal r =
      new PathReal(new File(tempdir, basename).toString() + ".7z");
    return new Archive7z(TestData.getLog(), r, mount);
  }
      ...
      IOException
  {
    ...
    try {
      final PathVirtual p = PathVirtual.ofString("/subdir");
      final Option<FileReference<Archive7zKind>> r = a.lookup(p);

      Assert.assertTrue(r.isSome());
      final Some<FileReference<Archive7zKind>> s =
        (Option.Some<FileReference<Archive7zKind>>) r;
